//! Vite web framework adapter
//!
//! Supports building Vite-based web applications (React, Vue, Svelte, vanilla JS, etc.).
...
impl ViteAdapter {
    ...
    fn get_dist_directory(&self, path: &Path) -> std::path::PathBuf {
        // Default Vite output is dist/
        // Could be customized in vite.config, but we'll use the default
        path.join("dist")
    }
}

impl Default for ViteAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl BuildAdapter for ViteAdapter {
    ...
    fn detect(&self, path: &Path) -> Detection {
        ...
        let has_vite_config = file_exists(path, "vite.config.js")
            ...
            || file_exists(path, "vite.config.mjs")
            || file_exists(path, "vite.config.mts");

        // Check for vite dependency in package.json
        let has_vite_dep = has_npm_dependency(path, "vite");

        if has_vite_config && has_vite_dep {
            Detection::Yes(95)
        } else if has_vite_config || has_vite_dep {
            Detection::Yes(70)
        } else {
            Detection::No
        }
    ...
    async fn build(&self, ctx: &BuildContext) -> Result<Vec<Artifact>> {
        ...
        let build_cmd = match ctx.profile {
            BuildProfile::Debug => "build",      // Vite doesn't have separate debug build
...
mod tests {
    ...
    #[test]
    fn test_adapter_id() {
        let adapter = ViteAdapter::new();
        assert_eq!(adapter.id(), "vite");
        assert_eq!(adapter.name(), "Vite");
    }
